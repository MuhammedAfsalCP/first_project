  ...
  return (
    <div className="w-full h-full p-6 flex flex-col">
      <div className="bg-white p-6 rounded-lg shadow-lg flex-1">
        ...
        {/* Delivered Button */}
        <button
          className={`px-4 py-2 mt-4 rounded-lg text-white ${spesificorder?.orderitems[0]?.status === 'Cancelled' || spesificorder?.orderitems[0]?.status === 'Delivered' ? 'hidden' : 'bg-green-500 self-end'
            }`}
          onClick={() => update()}
        >
          Delivered
